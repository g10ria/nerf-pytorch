...
def run_network(inputs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
...
#         raw = run_network(pts, fn=run_fn)
        raw = network_query_fn(pts, run_fn)
...
def train():
    ...
    N_iters = 1000 + 1
    print('Begin')
    print('POSES shape is', poses.shape)
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_video==0 and i > 0:
            ...
            savedir = os.path.join(basedir, expname, 'output_test')
            os.makedirs(savedir, exist_ok=True)
            with torch.no_grad(): # jellyfish
                rgbs, _ = render_path(render_poses[0:1,:,:], hwf, K,args.chunk, render_kwargs_test, savedir=savedir, gt_imgs=images, render_factor=args.render_factor)
